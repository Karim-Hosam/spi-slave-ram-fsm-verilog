...
clear settings -all
clear directives
#configure output directory  D:/1-Work/3-Digital_Design_Kareem_Wassem/2-Final_Projects/Project_2_SPI/spi-slave-ram-fsm-verilog/questalint
clear settings  -lib
#configure output directory  D:/1-Work/3-Digital_Design_Kareem_Wassem/2-Final_Projects/Project_2_SPI/spi-slave-ram-fsm-verilog/questalint
#clear directives 
...
#configure output directory  D:/1-Work/3-Digital_Design_Kareem_Wassem/2-Final_Projects/Project_2_SPI/spi-slave-ram-fsm-verilog/questalint
lint run -d SPI_Slave_with_Ram -L work
set qsDebug { lint_gui_mode } 
# 
# Questa Static Verification System
# Version 2021.1 4558100 win64 28-Jan-2021
...
lint report check -severity error assign_chain 
lint report check -severity info assign_int_to_real 
...
lint report check -severity info data_type_std_ulogic 
lint report check -severity warning delay_has_x_z 
lint report check -severity info delay_in_non_flop_expr 
lint report check -severity warning delay_negative 
lint report check -severity warning delay_not_int 
lint report check -severity warning delay_var 
lint report check -severity error delay_without_timescale 
...
lint report check -severity warning flop_async_reset_const 
...
lint report check -severity error for_stmt_with_complex_logic 
...
lint report check -severity info inout_port_exists 
lint report check -severity info inout_port_not_read 
...
lint report check -severity info module_output_not_registered 
...
lint report check -severity warning reset_port_connection_static 
...
lint report check -severity warning string_has_control_char 
lint report check -severity info subprogram_calling_subprogram 
lint report check -severity info subprogram_nested 
lint report check -severity info subprogram_unused 
...
lint report check -severity error timescales_differ 
...
lint report check -severity info undriven_output_port 
...
lint report check -severity info var_set_not_read 
lint report check -severity info var_unused 
lint report check -severity warning when_else_nested 
lint report check -severity warning while_loop_iteration_limit 
lint methodology soc -goal release 
lint run -d SPI_Slave_with_Ram -L work
